...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn("text-card-foreground grid auto-rows-min gap-2 rounded-lg p-1.5 bg-pop", className)}
      {...props}
    />
  )
}
...
function CardContent({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="card-content" className={cn("p-3 py-2 rounded bg-card", className)} {...props} />
}
